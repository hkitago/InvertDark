function show(platform, enabled, useSettingsInsteadOfPreferences) {
  ...
  const labelStrings = {
    "ar": {
      "iOS": "يمكنك تفعيل إضافة عكسالظلام على Safari من الإعدادات. يرجى ملاحظة أنك بحاجة إلى فتح إعدادات Safari يدويًا. بالنقر على الزر أدناه، سيتم إعادة توجيهك إلى صفحة إعدادات التطبيق. لتمكين الإضافة، انتقل إلى إعدادات Safari وقم بتفعيلها.",
    ...
    "en": {
      "iOS": "You can turn on InvertDark's Safari extension in Settings. Please note that you need to open Safari settings manually. By tapping the button below, you will be redirected to the app's settings page. To enable the extension, go to Safari's settings and activate it.",
    ...
    "es": {
      "iOS": "Puedes activar la extensión Safari de InvertirOscuro en la Configuración. Ten en cuenta que necesitas abrir la configuración de Safari manualmente. Al tocar el botón de abajo, serás redirigido a la página de configuración de la aplicación. Para habilitar la extensión, ve a la configuración de Safari y actívala.",
    ...
    "hi": {
      "iOS": "आप सेटिंग्स में उल्टाअंधेरा की Safari एक्सटेंशन को चालू कर सकते हैं। कृपया ध्यान दें कि आपको Safari सेटिंग्स को मैन्युअल रूप से खोलना होगा। नीचे दिए गए बटन पर टैप करके, आप ऐप की सेटिंग्स पेज पर पुनर्निर्देशित किए जाएंगे। एक्सटेंशन को सक्षम करने के लिए, Safari की सेटिंग्स में जाएं और इसे सक्रिय करें।",
    ...
    "id": {
      "iOS": "Anda dapat mengaktifkan ekstensi Safari InvertGelap di Pengaturan. Harap perhatikan bahwa Anda perlu membuka pengaturan Safari secara manual. Dengan mengetuk tombol di bawah, Anda akan diarahkan ke halaman pengaturan aplikasi. Untuk mengaktifkan ekstensi, buka pengaturan Safari dan aktifkan.",
    ...
    "ja": {
      "iOS": "設定で反転ダークモードのSafari拡張機能を有効にすることができます。Safari設定は手動で開く必要があることにご注意ください。下のボタンをタップすると、アプリの設定ページにリダイレクトされます。拡張機能を有効にするには、Safariの設定に移動して有効化してください。",
    ...
    "ko-KR": {
      "iOS": "설정에서 인버트다크의 Safari 확장을 활성화할 수 있습니다. Safari 설정을 수동으로 열어야 한다는 점에 유의하세요. 아래 버튼을 탭하면 앱의 설정 페이지로 리디렉션됩니다. 확장 프로그램을 활성화하려면 Safari 설정으로 이동하여 활성화하세요.",
    ...
    "nl": {
      "iOS": "U kunt de InvertDonker Safari-extensie inschakelen in Instellingen. Let op: u moet de Safari-instellingen handmatig openen. Door op de onderstaande knop te tikken, wordt u doorgestuurd naar de instellingenpagina van de app. Om de extensie in te schakelen, gaat u naar de Safari-instellingen en activeert u deze.",
    ...
    "pt-PT": {
      "iOS": "Pode ativar a extensão do Safari do InverterEscuro nas Definições. Note que precisa de abrir as definições do Safari manualmente. Ao tocar no botão abaixo, será redirecionado para a página de definições da aplicação. Para ativar a extensão, vá às definições do Safari e ative-a.",
    ...
    "th": {
      "iOS": "คุณสามารถเปิดใช้งานส่วนขยาย Safari ของ อินเวิร์ตดาร์ก ได้ในการตั้งค่า โปรดทราบว่าคุณจำเป็นต้องเปิดการตั้งค่า Safari ด้วยตนเอง เมื่อแตะปุ่มด้านล่าง คุณจะถูกนำไปยังหน้าการตั้งค่าของแอป เพื่อเปิดใช้งานส่วนขยาย ให้ไปที่การตั้งค่า Safari และเปิดใช้งาน",
    ...
    "tr": {
      "iOS": "TersKaranlık'nın Safari uzantısını Ayarlar'da açabilirsiniz. Safari ayarlarını manuel olarak açmanız gerektiğini lütfen unutmayın. Aşağıdaki düğmeye dokunarak, uygulamanın ayarlar sayfasına yönlendirileceksiniz. Uzantıyı etkinleştirmek için Safari ayarlarına gidin ve etkinleştirin.",
    ...
    "zh": {
      "iOS": "您可以在设置中启用 反转暗黑 的 Safari 扩展程序。请注意，您需要手动打开 Safari 设置。点击下方按钮，您将被重定向到应用程序的设置页面。要启用扩展程序，请进入 Safari 设置并激活它。",
    ...
    "zh-TW": {
      "iOS": "您可以在設定中啟用 反轉暗黑 的 Safari 擴充功能。請注意，您需要手動開啟 Safari 設定。點擊下方按鈕，您將被重新導向到應用程式的設定頁面。要啟用擴充功能，請進入 Safari 設定並啟用它。",
    ...
    "zh-HK": {
      "iOS": "您可以在設定中啟用 反轉暗黑 的 Safari 擴展程式。請注意，您需要手動開啟 Safari 設定。點擊下方按鈕，您將被重新導向到應用程式的設定頁面。要啟用擴展程式，請進入 Safari 設定並啟用它。",
    ...
    "it": {
      "iOS": "Puoi attivare l'estensione Safari di InvertiScuro nelle Impostazioni. Tieni presente che devi aprire manualmente le impostazioni di Safari. Toccando il pulsante qui sotto, verrai reindirizzato alla pagina delle impostazioni dell'app. Per abilitare l'estensione, vai alle impostazioni di Safari e attivala.",
    ...
    "uk": {
      "iOS": "Ви можете увімкнути розширення Safari ІнвертТемрява у Налаштуваннях. Зверніть увагу, що вам потрібно вручну відкрити налаштування Safari. Натиснувши кнопку нижче, ви перейдете на сторінку налаштувань додатка. Щоб увімкнути розширення, перейдіть до налаштувань Safari та активуйте його.",
    ...
    "ca": {
      "iOS": "Podeu activar l'extensió de Safari de InvertFosc a Configuració. Tingueu en compte que heu d'obrir manualment la configuració de Safari. En tocar el botó de sota, sereu redirigits a la pàgina de configuració de l'aplicació. Per habilitar l'extensió, aneu a la configuració de Safari i activeu-la.",
    ...
    "hr": {
      "iOS": "Možete uključiti InvertTamno Safari proširenje u Postavkama. Imajte na umu da trebate ručno otvoriti Safari postavke. Dodirom na gumb ispod bit ćete preusmjereni na stranicu postavki aplikacije. Da biste omogućili proširenje, idite u Safari postavke i aktivirajte ga.",
    ...
    "sv": {
      "iOS": "Du kan aktivera Safari-tillägget för InvertMörker i Inställningar. Observera att du måste öppna Safari-inställningarna manuellt. Genom att trycka på knappen nedan kommer du att omdirigeras till appens inställningssida. För att aktivera tillägget, gå till Safari-inställningarna och aktivera det.",
    ...
    "sk": {
      "iOS": "Môžete aktivovať rozšírenie Safari pre InvertTmavý v Nastaveniach. Upozorňujeme, že nastavenia Safari musíte otvoriť manuálne. Kliknutím na tlačidlo nižšie budete presmerovaní na stránku nastavení aplikácie. Pre povolenie rozšírenia prejdite do nastavení Safari a aktivujte ho.",
    ...
    "cs": {
      "iOS": "Můžete zapnout rozšíření Safari pro InvertTmavý v Nastavení. Vezměte prosím na vědomí, že musíte otevřít nastavení Safari ručně. Klepnutím na tlačítko níže budete přesměrováni na stránku nastavení aplikace. Chcete-li povolit rozšíření, přejděte do nastavení Safari a aktivujte jej.",
    ...
    "da": {
      "iOS": "Du kan aktivere Safari-udvidelsen for InvertMørk i Indstillinger. Bemærk venligst, at du skal åbne Safari-indstillingerne manuelt. Ved at trykke på knappen nedenfor vil du blive omdirigeret til appens indstillingsside. For at aktivere udvidelsen skal du gå til Safari-indstillingerne og aktivere den.",
    ...
    "nb": {
      "iOS": "Du kan aktivere Safari-utvidelsen for InvertMørk i Innstillinger. Vær oppmerksom på at du må åpne Safari-innstillingene manuelt. Ved å trykke på knappen nedenfor vil du bli omdirigert til appens innstillingsside. For å aktivere utvidelsen, gå til Safari-innstillingene og aktiver den.",
    ...
    "hu": {
      "iOS": "Az InvertSötét Safari-bővítményét az Beállításokban kapcsolhatja be. Kérjük, vegye figyelembe, hogy a Safari beállításait manuálisan kell megnyitnia. Az alábbi gombra koppintva átirányítjuk az alkalmazás beállítások oldalára. A bővítmény engedélyezéséhez lépjen a Safari beállításaiba, és aktiválja azt.",
    ...
    "fi": {
      "iOS": "Voit aktivoida KäännäTummain Safari-laajennuksen Asetuksissa. Huomaa, että sinun on avattava Safari-asetukset manuaalisesti. Napauttamalla alla olevaa painiketta sinut ohjataan sovelluksen asetussivulle. Ota laajennus käyttöön siirtymällä Safarin asetuksiin ja aktivoimalla se.",
    ...
    "vi": {
      "iOS": "Bạn có thể bật tiện ích mở rộng Safari của ĐảoNgược Tối trong Cài đặt. Xin lưu ý rằng bạn cần mở cài đặt Safari thủ công. Bằng cách nhấn vào nút bên dưới, bạn sẽ được chuyển hướng đến trang cài đặt của ứng dụng. Để bật tiện ích mở rộng, hãy vào cài đặt Safari và kích hoạt nó.",
    ...
    "he": {
      "iOS": "אתה יכול להפעיל את תוסף Safari של היפוך חושך בהגדרות. שים לב שעליך לפתוח את הגדרות Safari באופן ידני. על ידי הקשה על הכפתור למטה, תועבר לדף ההגדרות של האפליקציה. כדי להפעיל את התוסף, עבור להגדרות Safari והפעל אותו.",
    ...
    "pl": {
      "iOS": "Możesz włączyć rozszerzenie Safari OdwróćCiemność w Ustawieniach. Pamiętaj, że musisz ręcznie otworzyć ustawienia Safari. Dotykając przycisku poniżej, zostaniesz przekierowany na stronę ustawień aplikacji. Aby włączyć rozszerzenie, przejdź do ustawień Safari i aktywuj je.",
    ...
    "ms": {
      "iOS": "Anda boleh mengaktifkan sambungan Safari SongsangGelap dalam Tetapan. Sila ambil perhatian bahawa anda perlu membuka tetapan Safari secara manual. Dengan mengetik butang di bawah, anda akan dialihkan ke halaman tetapan aplikasi. Untuk membolehkan sambungan, pergi ke tetapan Safari dan aktifkannya.",
    ...
    "ro": {
      "iOS": "Puteți activa extensia Safari InvertÎntunecat în Setări. Vă rugăm să rețineți că trebuie să deschideți manual setările Safari. Apăsând butonul de mai jos, veți fi redirecționat către pagina de setări a aplicației. Pentru a activa extensia, accesați setările Safari și activați-o.",
    ...
    "ru": {
      "iOS": "Вы можете включить расширение Safari ИнвертТьма в Настройках. Обратите внимание, что вам нужно вручную открыть настройки Safari. Нажав на кнопку ниже, вы будете перенаправлены на страницу настроек приложения. Чтобы включить расширение, перейдите в настройки Safari и активируйте его.",
